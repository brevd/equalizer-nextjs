...
import { useOptimisticSplits } from "@/app/(app)/admin/expenses/[expenseId]/splits/useOptimisticSplits";
      ...
      {optimisticSplits.length === 0 ? (
        ...
        <ul>
          {optimisticSplits.map((split) => (
            <Split split={split} key={split.id} openModal={openModal} />
          ))}
        </ul>
      )}
    </div>
  ...
  return (
    ...
    >
      ...
      <Button variant={"link"} asChild>
        <Link href={basePath + "/" + split.id}>Edit</Link>
      </Button>
    </li>
  );
};
    ...
    <div className="text-center">
      ...
      <div className="mt-6">
        <Button onClick={() => openModal()}>
          <PlusIcon className="h-4" /> New Splits{" "}
        </Button>
      </div>
    </div>
  );
